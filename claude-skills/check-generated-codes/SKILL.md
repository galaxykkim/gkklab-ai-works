...
description: 현재 세션에서 Claude Code가 생성/수정한 소스 파일을 자동으로 감지하여 클린아키텍처 기반의 MVVM / MVI / TCA 패턴 준수 여부를 검사하고 보완 여부를 사용자에게 확인받는 스킬.
tools: AskUserQuestion, Read, Edit, Bash
---

# Check Generated Codes

현재 세션에서 Claude Code가 생성하거나 수정한 소스 파일을 자동으로 탐지하고,
클린아키텍처 원칙과 선택한 UI 패턴(MVVM / MVI / TCA)을 준수하는지 검사한다.

---

## 실행 절차

### Step 1: Claude Code가 생성/수정한 파일 자동 감지

Bash 도구로 현재 프로젝트의 git 변경 내역을 조회하여 소스 파일 목록을 수집한다.

```bash
git status --porcelain 2>/dev/null
```

출력 결과에서 각 줄의 상태 코드와 파일 경로를 파싱한다.
- 상태 코드: `A`, `M`, `AM`, `??` 등 (추가 또는 수정된 파일)
- 파일 경로: 줄의 3번째 문자 이후

수집한 파일 중 아래 조건을 모두 만족하는 파일만 `TARGET_FILES`에 포함한다:

**포함 조건 (확장자 중 하나):**
- `.dart`, `.swift`, `.kt`, `.tsx`, `.ts`
...
`git status` 실패 시 아래 메시지를 출력하고 종료한다:
```
❌ git 저장소를 찾을 수 없습니다. 프로젝트 루트에서 실행하세요.
```

#### TARGET_FILES가 비어 있는 경우

아래 메시지를 출력하고 종료한다:
```
❌ 감지된 소스 파일이 없습니다. Claude Code로 생성하거나 수정한 파일이 있는지 확인하세요.
```

#### 감지 결과 확인

AskUserQuestion 도구로 감지된 파일 목록을 사용자에게 보여주고 계속 진행할지 확인한다.
...
이 파일들을 검사할까요? (예/아니오)
일부만 검사하려면 번호를 쉼표로 입력하세요. (예: 1,3)
```

- `예` 또는 `y` → 전체 파일 검사
- 숫자 목록(예: `1,3`) → 해당 번호 파일만 검사
- `아니오` 또는 `n` → 종료
